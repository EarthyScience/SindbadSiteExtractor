...
class jung_QC:
    ...
    def process(self):
        if self.temporal_resolution != 'daily':
            logger.warning(f"jung_QC only provides daily data but the resolution is {self.temporal_resolution}. The QC flag will not be included in {self.temporal_resolution} data for {self.site}.")
            return None
        src_prov    = JungQCProvider(cubepath=self.flx_cubepath, version=self.version, site=self.site)
