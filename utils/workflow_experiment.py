...
import copy
from providers.cliff_gapfill import cliff_gapfill
...
def load_provider(mod_name):
    mod = importlib.import_module("providers."+mod_name)
    return mod
...
def add_date_variables(dat):
    ...
    # logging.info(dates, ,, dates.day.to_numpy(), )
    for _ind in range(len(year)):
        data_date = date(year[_ind], month[_ind], day[_ind])
        delta = data_date - julian_base_date
        # logging.info(delta.days)
        julian_day[_ind] = delta.days
        # logging.info(year[_ind], month[_ind], day[_ind], julian_day[_ind])
        
    x_year = xr.Variable(["time"], year, attrs={'long_name': 'year', '_FillValue': -9999})
    dat=dat.assign(year=x_year)
    x_month = xr.Variable(["time"], month, attrs={'long_name': 'month of year', '_FillValue': -9999})
    dat=dat.assign(month=x_month)
    x_day = xr.Variable(["time"], day, attrs={'long_name': 'day of year', '_FillValue': -9999})
    dat=dat.assign(day=x_day)
    x_hour = xr.Variable(["time"], hour, attrs={'long_name': 'hour of day', '_FillValue': -9999})
    dat=dat.assign(hour=x_hour)
    x_julian_day = xr.Variable(["time"], julian_day, attrs={'long_name': f'days since {julian_base_date_str}', 'FillValue': np.nan})
            ...
            if var_ in plotvars:
                ...
                if ('depth_FLUXNET' in dat_.dims) & ('time' in dat_.dims):
                    ...
                    for layer_ in dat_.depth_FLUXNET.values:      
                        ax.plot(dat_.sel(depth_FLUXNET = layer_).time.values.reshape(-1), dat_.sel(depth_FLUXNET = layer_).values.reshape(-1), lw=0.6, label = 'Layer '+ str(layer_))
                    plt.legend()
                    ax.set_xlabel('time', size=8)
                    ax.set_ylabel(data_freq + " "+ var_ + ' [' + dat_.units + ']', size=8)
                elif ('time' in dat_.dims) & ('depth_FLUXNET' not in dat_.dims) & ('depth_soilGrids' not in dat_.dims):
                    fig, ax = plt.subplots(1, 1, figsize=(8, 4), gridspec_kw={'wspace': 0.35, 'hspace': 0.5})
                    if not var_.endswith("_gfld"):
                        ax.plot(dat_.time.values.reshape(-1), dat_.values.reshape(-1), lw=0.6)
                    else:
                        # print(dat_, dat_.sourceDataProductName)
                        ax.plot(dat_.time.values.reshape(-1), dat_.values.reshape(-1), lw=0.6, label=dat_.sourceDataProductName)
                        dat_ori_=ds[var_info['with_gaps']]
                        # print(dat_ori_)
                        ax.plot(dat_ori_.time.values.reshape(-1), dat_ori_.values.reshape(-1), lw=0.6, label=dat_ori_.sourceDataProductName)
                        plt.legend()
                ...
                plt.title(f'{site_info["site_ID"]}:: {data_key} in json: {dat_.sourceDataProductName} with {exp_settings["FLUXNET_version"]}\n {get_bounds_str(dat_,var_info["bounds"])}', multialignment='center', va='top',size=8, y=1.03)
                plt.savefig(site_fig_dir + "/" + site_info["site_ID"] + '_' + var_ + "." + exp_settings['start_date'].split('-')[0] + "-" + str(int(exp_settings['end_date'].split('-')[0])) + '.' + data_freq + '.png', dpi=300)
            else:
                logging.warning(f'Variable {var_} from {data_key} dataset was not found in processed data.')
    logging.info(f'Plotted data: {data_freq} | {site}')
    return

def add_gapfill_settings(exp_settings, fill_src=None, fill_tar=None,dataset_key=None, gap_filled_vars=None):
    exp_settings['sel_datasets']=np.append(exp_settings['sel_datasets'], dataset_key)
    exp_settings['dataset'][dataset_key] = copy.deepcopy(exp_settings['dataset'][fill_tar])
    ...
    for _v_tar in _v_tars:
        ...
        if _v_src in src_info.keys():
            gfl_var_info['sourceDataProductName'] = f"{tar_info[_v_tar]['sourceDataProductName']} gapfld-w {src_info[_v_src]['sourceDataProductName']}"
            gfl_var_info['publication'] = f"{tar_info[_v_tar]['publication']}; {src_info[_v_src]['publication']}"
            gfl_var_info['description'] = f"{tar_info[_v_tar]['sourceDataProductName']}:: {tar_info[_v_tar]['description']};{src_info[_v_src]['sourceDataProductName']}:: {src_info[_v_src]['description']}"
            gfl_var_info['data_path'] = f"{tar_info[_v_tar]['data_path']}; {src_info[_v_src]['data_path']}"
            gfl_var_info['with_gaps'] = _v_tar

        exp_settings['dataset'][dataset_key]['variables'][_v_tar_gfld] = gfl_var_info
...
def write_netcdf(ds, exp_settings, site, resampled=None):
    ...
    ncfile_name = os.path.join(ncdir, site + "." + exp_settings['start_date'].split('-')[0] + "." + str(int(exp_settings['end_date'].split('-')[0])) + '.' + data_freq + '.nc')
    ds.to_netcdf(ncfile_name, mode='w')
    logging.info(f'saved nc data: {data_freq} : {ncfile_name}')
    return
...
def harmonize_data_attrs(site_info, data, _config, resampled=None):
    ...
    for source_ in _config["sel_datasets"]:
        ...
        for var_ in var_selected:
            ...
            data[var_] = data[var_].assign_attrs(
                                            bounds = str([np.nanmin(data[var_]), np.nanmax(data[var_])]),
                                            units = var_info[var_]['variableUnit'],
                                            short_name = var_info[var_]['nameShort'],
                                            long_name = nameLong,
                                            sourceDataProductName = sourceDataProductName,
                                            publication = var_info[var_]['publication'], description=var_info[var_]['description'],
                                            data_path = var_info[var_]['data_path'])
            # print(var_, data[var_].attrs)
            # print('--------')
    ...
    if resampled is None:
        ...
        if _config["temporal_resolution"] == 'daily':
            date_base = datetime(int(sdate[0]), int(sdate[1]), int(sdate[2]), 00, 00, 00)
            date_arr = np.array([date_base + timedelta(days=i) for i in range(len(data['time']))])
            data['time'] = date_arr.astype(np.datetime64)
        if _config["temporal_resolution"] == 'hourly':
            date_base = datetime(int(sdate[0]), int(sdate[1]), int(sdate[2]), 00, 00, 00)
            date_arr = np.array([date_base + timedelta(hours=i) for i in range(len(data['time']))])
            data['time'] = date_arr.astype(np.datetime64)

                
    #%% Add global attributes
    CurrentScript = os.path.basename(__file__)
    data = data.assign_attrs(title = f'site dataset for {_config["exp_name"]}',
                        FLUXNET_version = _config["FLUXNET_version"],
                        temporal_resolution = data_freq,
                        last_disturbance_on = _config['last_disturbance_on'],
                        PFT = _config['sitePFT'],
                        PFT_provider = _config['PFTprovider'],
    ...
    logging.info(f'harmonized Attrs: {data_freq} | {site}')
...
def compile_site_data(site, exp_settings=None):
    log_path = os.path.join(exp_settings['OutPath']['log'], f"{site}_{exp_settings['OutPath']['expName']}.log")
    ...
    print('----------------sites---------------------------\n'
        f'Initialization of the data processing: {site}\n'
        '-------------------------------------------')
    ...
    if exp_settings["gap_fill"]['do_fill'] == True:
        for gfik, gfiv in exp_settings["gap_fill"].items():
            if isinstance(gfiv, dict):
                data_gapfilled = cliff_gapfill(fill_src=gfiv['source'], fill_tar=gfiv['target'], site_info=site_info,data_dict=data_all, qc_thres=gfiv['qc_thres'], config=exp_settings).process()
                gap_filled_vars = data_gapfilled.variables
                exp_settings = add_gapfill_settings(exp_settings,fill_src=gfiv['source'], fill_tar=gfiv['target'], dataset_key=f'{gfik}_gfld', gap_filled_vars=gap_filled_vars)
    ...
    exp_settings['PFTprovider']=PFTprovider
    #%% Merge all data
    ...
    try:
        ...
        logging.info(f"Cannot compile data for: {site_info['site_ID']} for  {site_info['site_ID']}. Error: ")
        print(e)
        logging.info(e)
        close_logger()
    ...
    ds = ds.sel(time = slice(exp_settings["start_date"], exp_settings["end_date"]))
    ...
    for res in res_harmon:
        if res is not None:
            ...
            ds_res = ds
        ds_res_h = harmonize_data_attrs(site_info, ds_res, exp_settings, resampled=res)
        write_netcdf(ds_res_h, exp_settings, site, resampled=res)
        if exp_settings['diagnostic_plots']:
            plot_dianostic_figures(ds_res_h, site_info, exp_settings, site, resampled=res)

    #%% Export site data
    site_dict_d = ds.to_dict(data=False).copy()
    site_dict = {site: {'start_year': exp_settings['start_date'].split('-')[0], 
    ...
    logging.info(f'Processing complete: {site_info["site_ID"]}')

    print('----------------site---------------------------\n'
        f'Successful data processing for site: {site}\n'
        '------------------------------------------------')
